...
% TODO: move to context of contribution?
% Motion control on humanoid robots, compared to other robot categories, is particularly challenging due to their narrow support surface, sparse mass distribution, redundancy due to the number of \acp{DoF}, and underactuation of their floating base.
% In this setting, the agent during the \ac{RL} training needs to navigate its state space properly balancing the exploration and the exploitation trade-off, preventing to obtain sub-optimal policies due to local minima.
...
The reward is a weighted sum of terms we categorised as regularisers, steady-state, and transient.
...
We evaluate the push-recovery performance from horizontal forces.
